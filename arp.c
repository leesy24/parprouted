...
int ipaddr_known(ARPTAB_ENTRY *list, struct in_addr addr, char *ifname) 
{
  ...
  else if (logenabled) syslog(LOG_DEBUG, "Did not find match for %s(%s)", inet_ntoa(addr), ifname);
...
void arp_reply(ether_arp_frame *reqframe, struct sockaddr_ll *ifs) 
{
  ...
  if (debug || logenabled) {
    ...
    dia.s_addr = *((long *)arp->arp_tpa);
     ...
     if (debug) printf("Replying to %s faking %s\n", inet_ntoa(sia), inet_ntoa(dia));
     else if (logenabled) syslog(LOG_DEBUG, "Replying to %s faking %s", inet_ntoa(sia), inet_ntoa(dia));
  }
...
void arp_req(char *ifname, struct in_addr remaddr, int gratuitous)
{
  ...
  if (debug) printf("Sending ARP request for %s to %s\n", inet_ntoa(remaddr), ifname);
  else if (logenabled) syslog(LOG_DEBUG, "Sending ARP request for %s to %s", inet_ntoa(remaddr), ifname);

  sendto(sock, &frame, sizeof(ether_arp_frame), 0, (struct sockaddr *) &ifs, sizeof(struct sockaddr_ll));
  close(sock);
...
void refresharp(ARPTAB_ENTRY *list)
{
  ...
  else if (logenabled) syslog(LOG_DEBUG, "Refreshing ARP entries.");
...
int rq_add(ether_arp_frame *req_frame, struct sockaddr_ll *req_if)
{
  ...
  if (req_queue_len > MAX_RQ_SIZE) {
    ...
    if (debug) printf("Request queue has grown too large, deleting last element\n");
    else if (logenabled) syslog(LOG_DEBUG, "Request queue has grown too large, deleting last element");
    temp = req_queue;
...
void rq_process(struct in_addr ipaddr, int ifindex) 
{
  ...
  while (cur_entry != NULL) {
      ...
      else if (logenabled) syslog(LOG_DEBUG, "Found %s in request queue", inet_ntoa(ipaddr));
...
void *arp(char *ifname) 
{
  ...
  while (1) {
    ...
    do {
      ...
      int ret;
      if ((ret = arp_recv(sock, &frame)) <= 0) {
        if (debug) printf("arp_recv(): ret=%d!\n", ret);
        else if (logenabled) syslog(LOG_DEBUG, "arp_recv(): ret=%d!", ret);
    	  continue;
      }

      /* Insert all the replies into ARP table */
      if (frame.arp.arp_op == htons(ARPOP_REPLY)) {
        ...
        memcpy(&k_arpreq.arp_ha.sa_data, &frame.arp.arp_sha, sizeof(frame.arp.arp_sha));
    	  ...
    	  if (debug) printf("Received reply: updating kernel ARP table for %s(%s).\n", inet_ntoa(sin->sin_addr), (char *) ifname);
        else if (logenabled) syslog(LOG_DEBUG, "Received reply: updating kernel ARP table for %s(%s).", inet_ntoa(sin->sin_addr), (char *) ifname);
      ...
      } // end of if (frame.arp.arp_op == ntohs(ARPOP_REPLY))
    ...
    if (debug) printf("Received ARP request for %s on iface %s\n", inet_ntoa(dia), (char *) ifname);
    else if (logenabled) syslog(LOG_DEBUG, "Received ARP request for %s on iface %s", inet_ntoa(dia), (char *) ifname);
    ...
    if (memcmp(&dia,&sia,sizeof(dia)) && dia.s_addr != 0) {
      ...
      else if (logenabled) syslog(LOG_DEBUG, "Adding %s to request queue", inet_ntoa(sia));
    	rq_add(&frame, &ifs);
      pthread_mutex_unlock(&arptab_mutex);
    }
  }
